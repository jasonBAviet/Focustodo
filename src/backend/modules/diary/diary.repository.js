...
export class DiaryRepository {
  async getDiaries(userId, filters) {
    ...
    const queryStr = `SELECT * FROM ${TABLE} ${where} ORDER BY position ASC, created_at DESC LIMIT $${limitParamIdx} OFFSET $${offsetParamIdx}`;
    const result = await pool.query(queryStr, params);
    const countResult = await pool.query(`SELECT count(*)::int AS total FROM ${TABLE} ${where}`, countParams);
  ...
  async reorderDiaries(userId, orderedIds) {
    ...
    try {
      ...
      await client.query(
        `UPDATE ${TABLE} t
         SET position = v.pos, updated_at = $2
         FROM (VALUES ${valuePlaceholders}) AS v(id, pos)
         WHERE t.id = v.id AND t.user_id = $1`,
        [userId, now, ...orderedIds]
      );
