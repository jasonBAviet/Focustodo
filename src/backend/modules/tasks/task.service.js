...
export class TaskService {
  ...
  async spawnNextOccurrence(taskRow, userId) {
    const repeat = taskRow.repeat ?? 'none';
    if (repeat === 'none') return null;
    const dueDate = taskRow.due_date ?? taskRow.dueDate ?? null;
    const repeatCustom = taskRow.repeat_custom ?? taskRow.repeatCustom ?? null;
    const nextDue = computeNextDue(dueDate, repeat, repeatCustom);
    if (!nextDue) return null;

    const tags = taskRow.tags ?? [];
    return await taskRepository.insertTask({
      title: taskRow.title,
      projectId: taskRow.project_id ?? taskRow.projectId ?? null,
      priority: taskRow.priority ?? 'none',
      dueDate: nextDue,
      reminder: null,
      repeat,
      repeatCustom,
      note: taskRow.note ?? '',
      subtasks: [],
      pomodoroEstimate: taskRow.pomodoro_estimate ?? taskRow.pomodoroEstimate ?? 1,
      flagged: false,
  ...
  async getTasks(userId, filters) {
    ...
    const result = await taskRepository.getTasks(userId, filters);
    return {
  ...
  async getTaskById(id, userId) {
    ...
    if (!task) throw new Error('Task khong tim thay');
    return { data: rowToTask(task) };
  }
  ...
  async createTask(userId, body) {
    ...
    if (body.projectId) {
      const proj = await taskRepository.getProjectById(body.projectId, userId);
      if (!proj) {
        throw new Error(`Project "${body.projectId}" khong ton tai hoac khong thuoc quyen so huu.`);
      }
    }

    const taskRow = await taskRepository.insertTask(body, userId);
    const task = rowToTask(taskRow);
    this.triggerWebhook(userId, 'task.created', task);
    return { data: task };
  }

  async completeTask(id, userId, completed) {
    return await taskRepository.runInTransaction(async (client) => {
      // Temporary workaround for transaction: we pass pool inside repository by default.
      // We will read task normally
      const cur = await taskRepository.getTaskById(id, userId);
      if (!cur) throw new Error('Task khong tim thay');

      const now = new Date().toISOString();
      const wasCompleted = cur.completed === true;
      const completedAt = completed ? cur.completed_at ?? now : null;

      const upd = await taskRepository.updateTaskCompleteStatus(id, userId, completed, completedAt, now);
      
      let spawned = null;
      if (completed && !wasCompleted && cur.repeat && cur.repeat !== 'none') {
        spawned = await this.spawnNextOccurrence(cur, userId);
      }
      const updatedTask = rowToTask(upd);
      if (completed && !wasCompleted) {
        this.triggerWebhook(userId, 'task.completed', updatedTask);
      } else {
        this.triggerWebhook(userId, 'task.updated', updatedTask);
      }
      return { data: updatedTask, spawned: spawned ? rowToTask(spawned) : null };
    });
  }
  ...
  async updateTask(id, userId, body) {
    ...
    const updated = {
      ...
      start_date: 'startDate' in body ? body.startDate : cur.start_date,
    ...
    const result = await taskRepository.updateTask(id, userId, updated);
    
    let spawned = null;
    if ('completed' in body && body.completed === true && !wasCompleted && cur.repeat && cur.repeat !== 'none') {
      spawned = await this.spawnNextOccurrence(cur, userId);
    }
    const updatedTask = rowToTask(result);
  ...
  async reorderTasks(userId, orderedIds) {
    ...
    await taskRepository.reorderTasks(userId, orderedIds);
    return { status: 'ok', count: orderedIds.length };
  }

  async deleteTask(id, userId) {
    const now = new Date().toISOString();
    const result = await taskRepository.deleteTask(id, userId, now);
    if (!result) throw new Error('Task khong tim thay');
    const deletedTask = rowToTask(result);
    this.triggerWebhook(userId, 'task.deleted', deletedTask);
    return { data: { id: deletedTask.id }, message: 'Da xoa task thanh cong.' };
  }
}
