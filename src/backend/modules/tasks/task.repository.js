...
export class TaskRepository {
  async getTasks(userId, filters) {
    ...
    const queryStr = `SELECT * FROM tasks ${where} ORDER BY position ASC, created_at DESC LIMIT $${limitParamIdx} OFFSET $${offsetParamIdx}`;
  ...
  async getNextTaskPosition(projectId, userId) {
  ...
  async insertTask(input, userId) {
    const now = new Date().toISOString();
    const projectId = input.projectId ?? null;
    const position = input.position ?? (await this.getNextTaskPosition(projectId, userId));
    ...
    const result = await pool.query(
      ...
      [
        ...
        position, now, null, now, input.isKnowledge ?? false, userId
      ]
    );
    return result.rows[0];
  }

  async updateTaskCompleteStatus(id, userId, completed, completedAt, now) {
    const result = await pool.query(
  ...
  async updateTask(id, userId, updated) {
    const result = await pool.query(
      `UPDATE tasks SET
  ...
  async deleteTask(id, userId, now) {
    const result = await pool.query(
      'UPDATE tasks SET is_deleted = true, updated_at = $2 WHERE id = $1 AND user_id = $3 AND (is_deleted = false OR is_deleted IS NULL) RETURNING *',
      [id, now, userId]
    );
    return result.rows[0];
  }
  ...
  async getWebhookSettings(userId) {
    const result = await pool.query(
      'SELECT webhook_url, webhook_enabled FROM settings WHERE user_id = $1 OR id = $1',
      [userId]
    );
    return result.rows[0];
  }
