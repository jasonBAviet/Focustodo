...
const DateTimePicker: React.FC<DateTimePickerProps> = ({
  ...
  const initDate = value ? new Date(value) : new Date(today.getTime() + 60 * 60 * 1000);
  const [viewYear, setViewYear] = useState(initDate.getFullYear());
  const [viewMonth, setViewMonth] = useState(initDate.getMonth());
  const [selectedDate, setSelectedDate] = useState<string | null>(value ? toLocalDateString(initDate) : null);
  ...
  const handleOk = () => {
    if (selectedDate) {
      ...
      const finalDate = new Date(y, m - 1, d, hour, minute);
        ...
        <div className="time-picker">
          <input
            ...
            onChange={(e) => {
              ...
              if (!isNaN(val)) setHour(Math.min(23, Math.max(0, val)));
            }}
          />
          <span>:</span>
          <input
            ...
            onChange={(e) => {
              const val = parseInt(e.target.value, 10);
              if (!isNaN(val)) setMinute(Math.min(59, Math.max(0, val)));
            }}
          />
        </div>
