...
async function ensureSchema() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS projects (
      ...
      is_visible BOOLEAN, task_count INTEGER, folder_id TEXT, created_at TEXT
    );
    CREATE TABLE IF NOT EXISTS folders (
      id TEXT PRIMARY KEY, name TEXT NOT NULL, color TEXT,
      project_ids JSONB, created_at TEXT
    );
    CREATE TABLE IF NOT EXISTS tags (
      id TEXT PRIMARY KEY, name TEXT NOT NULL, color TEXT, created_at TEXT
    );
    CREATE TABLE IF NOT EXISTS tasks (
      id TEXT PRIMARY KEY, title TEXT NOT NULL, project_id TEXT,
      priority TEXT, due_date TEXT, reminder TEXT, repeat TEXT, repeat_custom TEXT,
      note TEXT, subtasks JSONB, pomodoro_estimate INTEGER, pomodoro_completed INTEGER,
      total_focus_time INTEGER, completed BOOLEAN, flagged BOOLEAN, tags JSONB,
      created_at TEXT, completed_at TEXT, updated_at TEXT
    );
    CREATE TABLE IF NOT EXISTS system_logs (
      id TEXT PRIMARY KEY, task_id TEXT, task_title TEXT, type TEXT,
      duration INTEGER, start_time TEXT, end_time TEXT, completed BOOLEAN
    );
    CREATE TABLE IF NOT EXISTS settings (
      id TEXT PRIMARY KEY, pomodoro_length INTEGER, short_break_length INTEGER,
      long_break_length INTEGER, long_break_after INTEGER, auto_start_next_pomodoro BOOLEAN,
      auto_start_break BOOLEAN, disable_break BOOLEAN, alarm_sound BOOLEAN,
      dark_mode TEXT, theme_wallpaper TEXT, accent_color TEXT, webhook_url TEXT,
      webhook_enabled BOOLEAN, external_api_url TEXT, external_api_enabled BOOLEAN,
      daily_focus_goal_hours NUMERIC, visible_views JSONB
    );
    CREATE TABLE IF NOT EXISTS ui_state (
      id TEXT PRIMARY KEY, selected_task_id TEXT, active_view TEXT,
      active_project_id TEXT, search_query TEXT
    );
  `);
}
...
function mapSettingsFromDb(r) {
  return {
    pomodoroLength: r.pomodoro_length, shortBreakLength: r.short_break_length,
    longBreakLength: r.long_break_length, longBreakAfter: r.long_break_after,
    autoStartNextPomodoro: r.auto_start_next_pomodoro, autoStartBreak: r.auto_start_break,
    disableBreak: r.disable_break, alarmSound: r.alarm_sound, darkMode: r.dark_mode,
    themeWallpaper: r.theme_wallpaper, accentColor: r.accent_color,
    webhookUrl: r.webhook_url, webhookEnabled: r.webhook_enabled,
    externalApiUrl: r.external_api_url, externalApiEnabled: r.external_api_enabled,
    dailyFocusGoalHours: Number(r.daily_focus_goal_hours), visibleViews: r.visible_views || {},
  };
}
...
async function handleGet(req, res) {
  const [tasksRes, projectsRes, foldersRes, tagsRes, logsRes, settingsRes, uiRes] =
    await Promise.all([
  ...
  const state = {
    ...
    projects: projectsRes.rows.map(r => ({
      id: r.id, name: r.name, color: r.color, isVisible: r.is_visible,
      taskCount: r.task_count, folderId: r.folder_id, createdAt: r.created_at,
    })),
    folders: foldersRes.rows.map(r => ({
      id: r.id, name: r.name, color: r.color,
      projectIds: r.project_ids || [], createdAt: r.created_at,
    })),
    tags: tagsRes.rows.map(r => ({
      id: r.id, name: r.name, color: r.color, createdAt: r.created_at,
    })),
    pomodoroSessions: logsRes.rows.map(r => ({
      id: r.id, taskId: r.task_id, taskTitle: r.task_title, type: r.type,
      duration: r.duration, startTime: r.start_time, endTime: r.end_time, completed: r.completed,
    })),
    webhookEvents: [],
    settings: settingsRes.rowCount > 0 ? mapSettingsFromDb(settingsRes.rows[0]) : undefined,
...
async function handlePost(req, res) {
  ...
  try {
    ...
    if (s.settings) {
      ...
      await client.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          pomodoro_length=EXCLUDED.pomodoro_length,short_break_length=EXCLUDED.short_break_length,
          long_break_length=EXCLUDED.long_break_length,long_break_after=EXCLUDED.long_break_after,
          auto_start_next_pomodoro=EXCLUDED.auto_start_next_pomodoro,auto_start_break=EXCLUDED.auto_start_break,
          disable_break=EXCLUDED.disable_break,alarm_sound=EXCLUDED.alarm_sound,
          dark_mode=EXCLUDED.dark_mode,theme_wallpaper=EXCLUDED.theme_wallpaper,
          accent_color=EXCLUDED.accent_color,webhook_url=EXCLUDED.webhook_url,
          webhook_enabled=EXCLUDED.webhook_enabled,external_api_url=EXCLUDED.external_api_url,
          external_api_enabled=EXCLUDED.external_api_enabled,
          daily_focus_goal_hours=EXCLUDED.daily_focus_goal_hours,visible_views=EXCLUDED.visible_views
    ...
    await client.query(`
      ...
      VALUES ($1,$2,$3,$4,$5)
      ON CONFLICT (id) DO UPDATE SET
        selected_task_id=EXCLUDED.selected_task_id,active_view=EXCLUDED.active_view,
        active_project_id=EXCLUDED.active_project_id,search_query=EXCLUDED.search_query
